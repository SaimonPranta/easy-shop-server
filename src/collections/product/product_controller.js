...
exports.product = async (req, res) => {
  try {
    ...
    if (search) {
      query = {
        $or: [
          ...
          {
            dis: new RegExp(search, "i"),
          },
          {
            "detailsArray.property.value": new RegExp(search, "i"),
          }, 
        ],
      };
    }
    ...
    const data = await product_collection.find(query);
    if (data.length) {
      res.status(200).json({
        data: data,
...
exports.addProduct = async (req, res) => {
  try {
    ...
    const { title, dis, price, discount, rating, detailsArray, viewAs } =
