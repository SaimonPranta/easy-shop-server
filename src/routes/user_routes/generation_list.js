...
const generation_list = async (req, res) => {
    try {
        ...
        if (generation == 1) {
            user.generation_1.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
            ...
            user.generation_2.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
            ...
            user.generation_3.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
            ...
            user.generation_4.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
            ...
            user.generation_5.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
            ...
            user.generation_6.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
            ...
            user.generation_7.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
            ...
            user.generation_8.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
            ...
            user.generation_9.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
            ...
            user.generation_10.map(async (uId) => {
                const currentUser = await user_collection.findOne({ _id: uId })
                currentUser.password = null;
                userArray.push(currentUser)
