...
const registation = async (req, res) => {
  try {
    const { firstName, lastName, phoneNumber, referNumber, password } =
      await req.body;
      ...
      if (refNumberChacking && refNumberChacking.length > 0) {
        if (refNumberChacking[0].isActive) {
          ...
          if (phoneNumberChacking && phoneNumberChacking.length > 0) {
            res.status(500).send({
              failed:
                "your porvided Phone Number are already exist, please tryout with another one",
            });
          } else {
            const userInfo = await {
              ...
              referUser: refNumberChacking[0]._id,
            ...
            if (createdUser.phoneNumber) {
              ...
              res.status(201).json({
                data: {
                  ...createdUser._doc,
                  referUser: {
                    phoneNumber: refNumberChacking[0].phoneNumber,
                    firstName: refNumberChacking[0].firstName,
                    lastName: refNumberChacking[0].lastName,
                  },
                },
              ...
              res.status(404).send({
                failed: "failed to Create your account, please tryout latter",
              });
            }
          }
        } else {
          res
            .status(500)
            .send({ failed: "your porvided reference Number are not Active" });
        }
      } else {
        res
          .status(500)
          .send({ failed: "your porvided reference Number are invalid" });
      }
    } else {
      res.status(500).send({
        failed: "your porvided Reference number & Phone Nmuber must be number",
      });
    }
  } catch (err) {
    res
      .status(404)
      .send({ failed: "failed to Create your account, please tryout latter" });
  }
};
