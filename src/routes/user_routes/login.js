  ...
  try {
    ...
    if (singInPhoenNumber && signInPassword) {
      const userArry = await user_collection.find({
        phoneNumber: singInPhoenNumber.toString(),
      });
      if (userArry.length > 0) {
          ...
          async (err, result) => {
            if (result) {
              ...
              if (
                userArry[0] &&
                userArry[0].block &&
                userArry[0].block.isBlock
              ) {
                return res
                  .status(401)
                  .json({ failed: "User account has been blocked" });
              }
              res.status(200).json({
                data: userArry[0],
                token: token,
              ...
              res
                .status(401)
                .json({
                  failed: "user/password are invalid, please try again.",
                });
            }
          }
        );
        ...
        res
          .status(401)
          .json({ failed: "user/password are invalid, please try again." });
      }
    } else {
      res
        .status(401)
